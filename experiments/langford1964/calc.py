...
def calc(rho_hat=0.95, bin_noise=0.33, mon_noise=0.70, calc=True,
    ...
    if not calc:
        ...
        all_data = pds.read_hdf(fname, 'langford1964')
        return all_data
    ...
    exp_data = exp_data.sort_values('tau')

    all_data = pds.DataFrame()
    for i, df in exp_data.iterrows():
        spi_dict = {'tau': df.tau * 1e-3,
                    'SNR': df.Spi,
                    'model': False,
                    'exp': 'Spi'}
        s0_dict = {'tau': df.tau * 1e-3,
                   'SNR': df.S0,
                   'model': False,
                   'exp': 'S0'}
    ...
    dtau = 0.1
    if not opt:
        tau_vals = np.arange(0, 9 + dtau, dtau) * 1e-3
    else:
    ...
    for i_tau, tau in enumerate(tau_vals):
        ...
        snr_s0 = find_snr(bin_noise=bin_noise,
                          ...
                          target_param=t_param,
                          reference_param=r_param)
        spi_dict = dict(tau=tau, SNR=10*np.log10(snr_spi[0]), model=True, exp='Spi',
                        bin_noise=bin_noise, mon_noise=mon_noise, rho_hat=rho_hat)
        s0_dict = dict(tau=tau, SNR=10*np.log10(snr_s0[0]), model=True, exp='S0',
                       bin_noise=bin_noise, mon_noise=mon_noise, rho_hat=rho_hat)
        all_data = all_data.append(pds.DataFrame([spi_dict]))
        all_data = all_data.append(pds.DataFrame([s0_dict]))
    ...
    if save:
        fname = 'data.h5' if save is True else save
        all_data.to_hdf(fname, 'langford1964')
    return all_data
